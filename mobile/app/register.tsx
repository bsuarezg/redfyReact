  ...
  return (
    <SafeAreaView className="flex-1 bg-white p-4">
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center' }}>
        <StyledView className="w-full max-w-sm self-center items-center">
          <StyledImage
          ...
          <StyledView className="mb-4">
            ...
            <StyledView className="flex-row space-x-4">
              <StyledTouchableOpacity
                className={`flex-1 p-3 rounded-lg border ${accountType === 'individual' ? 'bg-blue-50 border-primary' : 'border-gray-300'}`}
                onPress={() => setAccountType('individual')}
              >
                <StyledText className={`text-center ${accountType === 'individual' ? 'text-primary font-bold' : 'text-gray-600'}`}>
                  Individual
                </StyledText>
              </StyledTouchableOpacity>

              <StyledTouchableOpacity
                className={`flex-1 p-3 rounded-lg border ${accountType === 'clinic' ? 'bg-blue-50 border-primary' : 'border-gray-300'}`}
                onPress={() => setAccountType('clinic')}
              >
                <StyledText className={`text-center ${accountType === 'clinic' ? 'text-primary font-bold' : 'text-gray-600'}`}>
                  Clinic
                </StyledText>
              </StyledTouchableOpacity>
            </StyledView>
          ...
          >
            <StyledView className={`h-6 w-6 border rounded mr-2 ${consent ? 'bg-primary border-primary' : 'border-gray-400'}`} />
